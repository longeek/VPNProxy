use vpn_proxy::client_logic::{
    UDP_FRAME_VERSION, check_http_basic_auth, pack_udp_frame, parse_http_connect_target,
    parse_tcp_line_target, socks_udp_build_reply, socks_udp_parse_request,
};
use vpn_proxy::server_logic::{DRAIN_THRESHOLD, PIPE_BUF_SIZE, RECV_BUF_SIZE, next_session_id};
...
fn build_tls_config(cli: &Cli) -> Arc<rustls::ClientConfig> {
    ...
    if let Some(ref ca_cert) = cli.ca_cert {
        ...
        for cert in certs {
            root_store.add(cert).expect("cannot add CA cert");
        }
    }
...
async fn open_tunnel(
    ...
    let server_name: ServerName<'_> = sni
        .unwrap_or(server)
        .try_into()
    ...
    for attempt in 0..=retries {
        ...
        let mut payload =
            serde_json::json!({ "auth": token, "host": target_host, "port": target_port });
        if proto != "tcp" {
            payload["proto"] = serde_json::Value::String(proto.to_string());
        }
        let bootstrap = serde_json::to_string(&payload).unwrap() + "\n";

        if let Err(e) = tls.write_all(bootstrap.as_bytes()).await {
            last_err = Some(e);
            continue;
        }
...
impl TunnelPool {
    ...
    async fn acquire(
        ...
        while let Some((mut tls, created)) = self.entries.pop() {
            ...
            let mut status_buf = vec![0u8; 64];
            if let Ok(Ok(n)) =
                tokio::time::timeout(Duration::from_secs(10), tls.read(&mut status_buf)).await
            {
                let status = String::from_utf8_lossy(&status_buf[..n]);
                if status.starts_with("OK") {
                    self.hits += 1;
...
async fn socks5_handshake(
    ...
    if proxy_user.is_some() {
        if !methods.contains(&0x02) {
            w.write_all(&[SOCKS_VERSION, 0xFF]).await?;
            w.flush().await?;
            return Err(std::io::Error::new(
        ...
        w.write_all(&[SOCKS_VERSION, 0x02]).await?;
        w.flush().await?;
        let mut auth_ver = [0u8; 1];
        r.read_exact(&mut auth_ver).await?;
        if auth_ver[0] != 0x01 {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                "bad auth version",
        ...
        r.read_exact(&mut ulen_buf).await?;
        let mut username_buf = vec![0u8; ulen_buf[0] as usize];
        r.read_exact(&mut username_buf).await?;
        let mut plen_buf = [0u8; 1];
        r.read_exact(&mut plen_buf).await?;
        let mut password_buf = vec![0u8; plen_buf[0] as usize];
        r.read_exact(&mut password_buf).await?;
        ...
        w.write_all(&[SOCKS_VERSION, 0x00]).await?;
        w.flush().await?;
    }

    let mut req = [0u8; 4];
    r.read_exact(&mut req).await?;
    let cmd = req[1];
    let atyp = req[3];
    ...
    let (host, port) = match atyp {
        ...
        0x03 => {
            ...
            let mut hb = vec![0u8; lb[0] as usize];
            r.read_exact(&mut hb).await?;
            let mut pb = [0u8; 2];
            r.read_exact(&mut pb).await?;
            (
...
async fn handle_socks_udp_relay(
    ...
    let mut tunnel = match pool.open(&cli, "0.0.0.0", 0, "udp").await {
        ...
        Err(e) => {
            warn!(
                "[sid={session_id}] UDP tunnel open failed from {:?}: {e}",
                peer
            );
    ...
    let reply_ip = if bind_ip == "0.0.0.0" || bind_ip == "::" {
        "127.0.0.1"
    } else {
        &bind_ip
    };
    ...
    {
        let (_r, mut w) = client.split();
        let reply_ip_parsed: std::net::IpAddr = reply_ip
            .parse()
    ...
    let pending: Arc<Mutex<HashMap<(String, u16), Vec<(String, u16)>>>> =
    ...
    let app_to_udp = tokio::spawn(async move {
        ...
        loop {
            ...
            }
            {
                ...
                if tunnel_w.write_all(&frame).await.is_err() {
                    break;
                }
                if tunnel_w.flush().await.is_err() {
                    break;
                }
            }
        }
...
async fn handle_socks_client(
    ...
    let (target_host, target_port, cmd) = match socks5_handshake(
        &mut client,
        cli.proxy_user.as_deref(),
        cli.proxy_pass.as_deref(),
    )
    .await
    {
        Ok(v) => v,
        Err(e) => {
            debug!(
                "[sid={session_id}] SOCKS handshake failed from {:?}: {e}",
                peer
            );
            let (_r, mut w) = client.split();
    ...
    info!(
        "[sid={session_id}] SOCKS CONNECT from {:?} to {target_host}:{target_port}",
        peer
    );
    ...
    info!(
        "[sid={session_id}] SOCKS CONNECT OK from {:?} to {target_host}:{target_port}",
        peer
    );
...
async fn handle_http_client(
    ...
    if cli.proxy_user.is_some() && cli.proxy_pass.is_some() {
        ...
        let mut attempts = 0;
        loop {
            if check_http_basic_auth(&header_buf[..total], user, pass) {
                break;
            }
            attempts += 1;
            if attempts >= 2 {
    ...
    debug!(
        "[sid={session_id}] HTTP CONNECT from {:?} to {target_host}:{target_port}",
        peer
    );
    ...
    let _ = client.flush().await;
    info!(
        "[sid={session_id}] HTTP CONNECT OK from {:?} to {target_host}:{target_port}",
        peer
    );
    relay_bidirectional(client, tunnel).await;
}

async fn handle_tcp_line_client(
    mut client: TcpStream,
    cli: Arc<CliInner>,
    pool: Arc<PooledTunnelOpener>,
) {
    ...
    let newline_pos = line_buf[..total]
        .iter()
        .position(|&b| b == b'\n')
        .unwrap_or(total);
    let (target_host, target_port) = match parse_tcp_line_target(&line_buf[..newline_pos]) {
        ...
        Err(e) => {
            warn!(
                "[sid={session_id}] TCP line parse failed from {:?}: {e}",
                peer
            );
    ...
    debug!(
        "[sid={session_id}] TCP line target from {:?} -> {target_host}:{target_port}",
        peer
    );
    ...
    let tunnel = match pool.open(&cli, &target_host, target_port, "tcp").await {
        ...
        Err(e) => {
            warn!(
                "[sid={session_id}] TCP line tunnel failed from {:?}: {e}",
                peer
            );
            let msg = format!("ERR {e}\n");
            let (_r, mut w) = client.split();
    ...
    let _ = w.flush().await;
    info!(
        "[sid={session_id}] TCP tunnel OK from {:?} to {target_host}:{target_port}",
        peer
    );
    relay_bidirectional(client, tunnel).await;
}
...
async fn main() {
    ...
    let inner = Arc::new(CliInner {
        ...
        proxy_pass: cli.proxy_pass.clone(),
    ...
    if cli.pool_size > 0 {
        ...
        pool.refill().await;
        info!(
            "tunnel pool started (size={}, ttl={:.1}s)",
            cli.pool_size, cli.pool_ttl
        );
    ...
    let pool_opener = Arc::new(PooledTunnelOpener {
        pool: pool_arc.clone(),
    });

    let socks_listener = TcpListener::bind((&cli.listen[..], cli.listen_port))
        .await
        .expect("cannot bind SOCKS5");
    let auth_info = if cli.proxy_user.is_some() {
        format!(", auth={}", cli.proxy_user.as_deref().unwrap())
    } else {
        String::new()
    };
    info!(
        "SOCKS5 proxy listening on {}:{}{}",
        cli.listen, cli.listen_port, auth_info
    );
    ...
    let socks_handle = tokio::spawn(async move {
        loop {
            ...
            tokio::spawn(async move {
                handle_socks_client(client, ic, p).await;
            });
        }
    });
    ...
    let http_handle = tokio::spawn(async move {
        if let Some(hp) = http_port {
            let listener = TcpListener::bind((&listen_for_http[..], hp))
                .await
                .expect("cannot bind HTTP");
            ...
            loop {
                ...
                tokio::spawn(async move {
                    handle_http_client(client, ic, p).await;
                });
            }
        }
    });
    ...
    let tcp_line_handle = tokio::spawn(async move {
        if let Some(tp) = tcp_line_port {
            let listener = TcpListener::bind((&listen_for_tcp[..], tp))
                .await
                .expect("cannot bind TCP line");
            ...
            loop {
                ...
                tokio::spawn(async move {
                    handle_tcp_line_client(client, ic, p).await;
                });
            }
        }
    });
